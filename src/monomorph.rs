use super::w_ds::{TypedNode, Type, Types, Scheme};
            ...
            match *e {
                ...
                Scheme::Forall(e, _) => {
                    ...
                    uses.into_iter().fold(monomorph(*b), |b, t| {
                        ...
                        Node::Let(i.clone(), Box::new(monomorph(e.clone().apply(&info))), Box::new(b))
                    })
                }
            }
        }
...
fn uses(s: &TypedNode, i: &str) -> HashSet<Type> {
    match s {
        ...
        &TypedNode::Let(_, ref e, ref b) => &uses(match &**e {
            &Scheme::Type(ref e) | &Scheme::Forall(ref e, _) => e,
        }, i) | &uses(b, i),
        &TypedNode::Match(ref a, ref arms, _) => {
            &uses(a, i) |
            &arms.iter()
                .flat_map(|&(ref p, ref b)| if p.bound_vars().contains(&Arg::Ident(String::from(i))) {
                    HashSet::new()
                } else {
                    uses(b, i)
                })
