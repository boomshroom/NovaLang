...
use super::super::w_ds::{self, TypedNode, Type};
...
pub fn compile(n: TypedNode) -> Result<String, NulError> {
	...
	let mut comp = Compiler{ ctx: &ctx, lmod: &modl, build: ctx.builder(), vars: HashMap::new() };
	comp.compile(n);
...
impl <'a> Compiler<'a> {
	...
	fn compile_app(&mut self, f: TypedNode, a: TypedNode) -> LLVMValueRef {
		...
		match f_ty {
			Type::Func(_, _) => {
				unsafe { LLVMBuildCall(*self.build, f, &mut a as *mut LLVMValueRef, 1, 0 as *const _) }
			},
			Type::Closure(_, _, _) => {
				let f_ptr = unsafe { LLVMBuildExtractElement(*self.build, f, wrap::uint(0, self.ctx.int_type(8)), null()) };
				let f_cap = unsafe { LLVMBuildExtractElement(*self.build, f, wrap::uint(1, self.ctx.int_type(8)), null()) };
				let mut args = [f_cap, a];
				unsafe { LLVMBuildCall(*self.build, f_ptr, args.as_mut_ptr(), args.len() as u32, 0 as *const _) }
			}
			_ => unreachable!(),
		}
	}

	fn compile_abs(&mut self, i: Option<Arg>, b: TypedNode, arg: Type, cap: Vec<(Arg, Type)>) -> LLVMValueRef {
		//let ll_type = self.ll_type(&b.get_type());
		match cap.len() {
			0 => {
				let ll_type = self.ll_type(&Type::Func(Box::new(arg), Box::new(b.get_type())));

				let f_obj = unsafe { LLVMAddFunction(**self.lmod, null(), ll_type) };
				//let f_obj = self.lmod.new_function("", ll_type).unwrap();
				let mut new_ctx = Compiler{ctx: self.ctx, lmod: self.lmod, build: self.ctx.builder(), vars: HashMap::new()};
				...
				i.map(|i| new_ctx.vars.insert(i, unsafe { LLVMGetParam(f_obj, 0) }));
			...
			_ => {
				let ll_type = self.ll_type(&Type::Closure(Box::new(arg), Box::new(b.get_type()), cap.iter().map(|&(_, ref t)| t.clone()).collect()));

				let capt : Vec<_> = cap.clone().into_iter().map(|(v, t)| self.compile_var(v, t)).collect();

				let f_ptr_ty = unsafe { LLVMGetElementType(LLVMStructGetTypeAtIndex(ll_type, 0)) };
				let f_obj = unsafe { LLVMAddFunction(**self.lmod, null(), f_ptr_ty) };
				...
				let vars : Vec<_> = cap.into_iter().enumerate().map(|(i, (v, _))| (v, unsafe { LLVMBuildExtractValue(*new_ctx.build, capts, i as u32, null()) })).collect();
				...
				let mut cls_vec = [f_obj, capt_obj];
				let cls_obj = unsafe { LLVMConstStructInContext(**self.ctx, cls_vec.as_mut_ptr(), cls_vec.len() as u32, 0) };

				let cap_built = capt.into_iter().enumerate().fold(capt_obj, |obj, (i, v)| unsafe { LLVMBuildInsertValue(*self.build, obj, v, i as u32, null()) });			
	...
	fn ll_type(&self, t: &Type) -> LLVMTypeRef {
		match *t {
			...
			Type::Func(ref a, ref r) => unsafe { LLVMPointerType(LLVMFunctionType(self.ll_type(r), &mut self.ll_type(a) as *mut _, 1, 0 ), 0) },
			Type::Closure(ref a, ref r, ref c) => {
				...
				let f_ty = unsafe { LLVMPointerType(LLVMFunctionType(self.ll_type(r), args.as_mut_ptr(), args.len() as u32, 0 ), 0) };
	...
	fn zero_for_type(&self, t: &Type) -> LLVMValueRef {
		match *t {
			...
			Type::Func(ref a, ref r) => unsafe { LLVMConstNull(LLVMPointerType(LLVMFunctionType(self.ll_type(r), &mut self.ll_type(a) as *mut _, 1, 0 ), 0)) },
			Type::Closure(ref a, ref r, ref c) => {
				...
				let f_ptr = unsafe { LLVMConstNull(LLVMPointerType(LLVMFunctionType(self.ll_type(r), args.as_mut_ptr(), args.len() as u32 , 0 ), 0)) };
