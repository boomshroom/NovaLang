...
impl Context {
    ...
    pub fn ll_type(&self,
        ...
        match *t {
            ...
            Type::Unit => unsafe { LLVMStructTypeInContext(**self, 0 as *mut LLVMTypeRef, 0, 0) },
            Type::Tuple(ref ts) => {
                let mut ts: Vec<_> =
                    ts.iter().map(|t| self.ll_type(t, types, structs, target)).collect();
                unsafe { LLVMStructTypeInContext(**self, ts.as_mut_ptr(), ts.len() as u32, 0) }
            }
            Type::Func(ref a, ref r) => unsafe {
                LLVMPointerType(
                    LLVMFunctionType(
                        self.ll_type(r, types, structs, target),
                        &mut self.ll_type(a, types, structs, target) as *mut _, 1, 0),
                    0,
                )
            },
            Type::Closure(ref a, ref r, ref c) => {
                let mut ts: Vec<_> =
                    c.iter().map(|t| self.ll_type(t, types, structs, target)).collect();
                let cap =
                    unsafe { LLVMStructTypeInContext(**self, ts.as_mut_ptr(), ts.len() as u32, 0) };
                let mut args = [cap, self.ll_type(a, types, structs, target)];
                let f_ty = unsafe {
                    LLVMPointerType(LLVMFunctionType(self.ll_type(r, types, structs, target),
                                                     args.as_mut_ptr(),
                                                     args.len() as u32,
                                                     0),
                                    0)
                ...
                unsafe {
                    LLVMStructTypeInContext(**self, closure.as_mut_ptr(), closure.len() as u32, 0)
                }
            }
            Type::Free(_) => {
            ...
            Type::Alias(ref i, ref args) => {
                ...
                let e = match types.get(i) {
                    Some(&Scheme::Type(EnumDecl(ref e))) => {
                        assert_eq!(args.len(), 0, "Type arguments given for non polymorphic type.");
                        e.clone()
                    }
                ...
                let mut ts = e.iter()
                    .map(|&(_, ref ts)| {
                        let mut ts =
                            ts.iter().map(|t| self.ll_type(t, types, structs, target)).collect::<Vec<_>>();
                        unsafe {
                            LLVMStructTypeInContext(**self, ts.as_mut_ptr(), ts.len() as u32, 0)
                        }
                    })
                    .collect::<Vec<_>>();
    ...
    fn union_type(&self, ts: Vec<LLVMTypeRef>, target: LLVMTargetDataRef) -> LLVMTypeRef {
        match ts.iter().map(|&t| unsafe { LLVMABISizeOfType(target, t) }).max() {
            None | Some(0) => unsafe { LLVMStructTypeInContext(**self, null(), 0, 0) },
            Some(size) => {
                let align =
                    match ts.iter().map(|&t| unsafe { LLVMABIAlignmentOfType(target, t) }).max() {
                        None => 1,
                        Some(n) => n as u32,
                    };
                assert!(size as u32 >= align, "Alignment greater than size!");
                unsafe {
                    LLVMStructTypeInContext(**self,
                                            [self.int_type(8 * align),
                                             LLVMArrayType(self.int_type(8),
                                                           (size as u32) - align)]
                                                .as_mut_ptr(),
                                            2,
                                            0)
